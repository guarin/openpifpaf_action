...
class Stanford40(DataModule):
    ...
    def __init__(self):
        ...
        action.upsample_stride = self.upsample_stride
        self.head_metas = [cif, caf, caf25, action]
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            ...
            help="""
                List of keypoint locations where action labels should be placed.
                Example: nose left_hip,right_hip
                This would create an action label on the nose and the center point between the left and right hip
                """,
        )
    ...
    def _encoders(self):
        return [
            encoder.ActionCif(self.head_metas[0], bmin=self.bmin),
            encoder.ActionCaf(self.head_metas[1], bmin=self.bmin),
            encoder.ActionCaf(self.head_metas[2], bmin=self.bmin),
            encoder.Action(self.head_metas[3], bmin=self.bmin),
        ]
        ...
        return openpifpaf.transforms.Compose(
            [
                ...
                openpifpaf.transforms.ToAnnotations(
                    [to_kp_annotations, to_action_annotations]
                ),
                openpifpaf.transforms.EVAL_TRANSFORM,
            ]
        )

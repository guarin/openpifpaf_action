...
class Crop(openpifpaf.transforms.Preprocess):
    ...
    def area_of_interest(anns, valid_area, edge_length):
        ...
        anns_of_interest = [
            ann
            for ann in anns
            if ("actions" in ann) and np.any(ann["keypoints"][:, 2] > 0)
        ]
        ...
        min_x = (
            min(
                np.min(ann["keypoints"][ann["keypoints"][:, 2] > 0, 0])
                for ann in anns_of_interest
            )
            - 50
        )
        min_y = (
            min(
                np.min(ann["keypoints"][ann["keypoints"][:, 2] > 0, 1])
                for ann in anns_of_interest
            )
            - 50
        )
        max_x = (
            max(
                np.max(ann["keypoints"][ann["keypoints"][:, 2] > 0, 0])
                for ann in anns_of_interest
            )
            + 50
        )
        max_y = (
            max(
                np.max(ann["keypoints"][ann["keypoints"][:, 2] > 0, 1])
                for ann in anns_of_interest
            )
            + 50
        )
